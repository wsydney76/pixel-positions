<x-layouts.app>
    <x-page-heading>Job Details</x-page-heading>

    <x-panel>
        <div class="mb-2 flex items-center gap-4">
            <div>
                <x-employer-logo width="60" :employer="$job->employer" />
            </div>
            <div class="text-lg text-gray-600">{{ $job->employer->name ?? 'N/A' }}</div>
        </div>

        <h1 class="mt-6 mb-4 text-3xl font-bold">{{ $job->title }}</h1>
        <div class="mb-2 text-gray-600">Location: {{ $job->location }}</div>
        <div class="mb-2 text-gray-600">Salary: {{ $job->salary }}</div>
        <div class="mb-2 text-gray-600">Schedule: {{ $job->schedule }}</div>
        <div class="mb-2 text-gray-600">
            Posted: {{ $job->created_at->format('F j, Y \a\t g:i A') }}
        </div>
        <div class="mb-4">
            <a href="{{ $job->url }}" class="text-blue-600 underline" target="_blank">
                Job Posting
            </a>
        </div>

        @if ($job->description)
            <div class="mb-6">
                <h2 class="mb-2 text-xl font-semibold">Description</h2>
                <div class="prose">{!! nl2br(e($job->description)) !!}</div>
            </div>
        @endif

        <div class="mb-4">
            <x-employer :employer="$job->employer" />
        </div>

        @if ($job->tags->count())
            <div class="mb-4">
                @foreach ($job->tags as $tag)
                    <x-tag :$tag />
                @endforeach
            </div>
        @endif
    </x-panel>

    <div class="space-x-4">
        @can('edit', $job)
            <a
                href="{{ route('jobs.edit', $job) }}"
                class="mt-4 inline-block text-blue-600 underline"
            >
                Edit
            </a>
            <form
                action="{{ route('jobs.destroy', $job) }}"
                method="POST"
                class="inline-block"
                onsubmit="return confirm('Are you sure you want to delete this job?');"
            >
                @csrf
                @method('DELETE')
                <button
                    type="submit"
                    class="mt-4 inline-block cursor-pointer text-red-600 underline"
                >
                    Delete
                </button>
            </form>
        @endcan

        <a href="{{ url()->previous() }}" class="mt-4 inline-block text-blue-600 underline">
            Back
        </a>
    </div>
</x-layouts.app>
